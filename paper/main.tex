...
\hypersetup{
  ...
  pdfkeywords={primordial power spectrum, CMB, inverse problems, Gaussian processes, Fredholm equations}
}

\title{\textbf{Reconstructing the primordial power spectrum with FrediPy:\\a proof of concept}}
\author{Kay Lehnert\\[2pt]
\small National University of Ireland, Maynooth\\
\small \href{mailto:k.lehnert@protonmail.com}{k.lehnert@protonmail.com}\quad
\href{https://orcid.org/0000-0002-2908-8638}{ORCID: 0000-0002-2908-8638}}
\date{August 2026}
...
lensing template, calibration and Gaussian-process hyperparameters fixed.  On
...
$0.49$ pointwise posterior standard deviations.  Their data quadratics are
$218.5$ and $218.9$, respectively.  FrediPy therefore passes the controlled
reconstruction test, while the observational example gives no reason to
replace a featureless power law in this conditional setting.  The latter is an
illustration, not a feature-significance or cosmological-parameter analysis.
\end{abstract}

\noindent\textbf{Keywords:} primordial power spectrum; CMB; inverse problems;
Gaussian processes; Fredholm equations
...
\begin{equation}
 ...
 \label{eq:rbf}
\end{equation}
Throughout this paper $m=1$, $\sigma_f=1$ and $\gamma=1.5$.  The mean is a
scale-invariant spectrum, not the tilted power law that we later reconstruct.
The prior permits smooth departures over order-unity intervals in $\ln k$.
We fix these values to keep the demonstration reproducible; they should be
varied or inferred before interpreting structure.
...
separate dense Cholesky evaluation of
...
GP hyperparameters & fixed RBF & fixed RBF \\
...
\end{equation}
We report comparisons where $s_j$ exceeds 1\% of its maximum.  This selects 89
contiguous nodes over
\begin{equation}
 0.004<k<0.20\,\Mpc^{-1}.
...
Their largest pointwise separation is $0.484$ posterior standard deviations,
...
experiment; and the kernel family and hyperparameters should be marginalised
or stress-tested.  Pointwise posterior bands are correlated and do not define
a global feature test.  The Gaussian prior also does not enforce positivity,
although the posterior means shown here remain positive.
...
OpenAI Codex was used in August 2026 to assist with repository restructuring,
numerical cross-checks and language editing under the author's supervision.
The scientific choices, verification and final manuscript remain the author's
responsibility.
